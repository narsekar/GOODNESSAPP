            ...
            <div className="space-y-6">
              ...
              <nav className="space-y-1">
                ...
                <button
                  ...
                  className={`w-full flex items-center justify-between px-3.5 py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer text-left border ${
                    activeTab === 'history'
                      ? 'bg-emerald-50 text-emerald-950 border-emerald-100 font-extrabold'
                      : 'bg-transparent text-slate-600 border-transparent hover:bg-slate-50 hover:text-slate-900'
                  }`}
                >
                  <div className="flex items-center gap-3">
                    <History size={15} className={activeTab === 'history' ? 'text-emerald-700' : 'text-slate-400'} />
                    <span>History Changes Log</span>
                  </div>
                ...
                >
                  <Users size={15} className={activeTab === 'directory' ? 'text-emerald-700' : 'text-slate-400'} />
              ...
              {activeTab === 'week_off' && (
                <div className="space-y-6">
                  ...
                  {session?.role === 'admin' ? (
                    ...
                    /* ---------------- STAFF SELF-SERVICE DASHBOARD & FORM ---------------- */
                    currentEmployee && (
                      <section className="grid grid-cols-1 md:grid-cols-3 gap-5">
                        
                        {/* A. Current Schedule Info Card */}
                        <div className="bg-gradient-to-br from-emerald-700 to-emerald-900 text-white rounded-2xl p-5 shadow-sm space-y-4 flex flex-col justify-between">
                          <div>
                            <div className="flex items-center gap-2">
                              <span className="w-1.5 h-1.5 rounded-full bg-emerald-300 animate-pulse" />
                              <span className="text-[9px] font-extrabold uppercase tracking-widest text-emerald-200">Registered Schedule</span>
                            </div>
                            <h3 className="text-lg font-black tracking-tight mt-1 truncate">{currentEmployee.name}</h3>
                            <p className="text-xs text-emerald-100 font-semibold">{currentEmployee.designation}</p>
                          </div>

                          <div className="grid grid-cols-2 gap-2 text-xs py-2 border-y border-emerald-600/40">
                            <div>
                              <span className="text-[10px] text-emerald-300 block">Emp ID:</span>
                              <span className="font-mono font-bold text-slate-100">{currentEmployee.id}</span>
                            </div>
                            <div>
                              <span className="text-[10px] text-emerald-300 block">Category:</span>
                              <span className="font-bold text-slate-100">
                                {currentEmployeeCollar === 'blue' ? 'Blue Collar' : 'White Collar'}
                              </span>
                            </div>
                            <div>
                              <span className="text-[10px] text-emerald-300 block">Duty Shift:</span>
                              <span className="font-bold text-slate-100">{currentEmployee.shift}</span>
                            </div>
                            <div>
                              <span className="text-[10px] text-emerald-300 block">Ward/Dept:</span>
                              <span className="font-bold text-slate-100">{currentEmployee.ward}</span>
                            </div>
                          </div>

                          <div className="bg-emerald-950/40 rounded-xl p-3 text-center border border-emerald-500/25">
                            <span className="text-[10px] text-emerald-200 block uppercase tracking-wider font-extrabold">Active Weekly Off Day</span>
                            <span className="text-lg font-extrabold text-white uppercase mt-0.5 block tracking-widest">{currentEmployee.weekOff}</span>
                          </div>
                        </div>

                        {/* B. Request/Fill New Week Off Form */}
                        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs flex flex-col justify-between gap-4">
                          <div>
                            <h3 className="text-sm font-bold text-slate-800 flex items-center gap-2">
                              <Calendar size={15} className="text-emerald-700" />
                              <span>Fill Week Off Update</span>
                            </h3>
                            <p className="text-xs text-slate-500 mt-1">
                              Propose your preferred weekly off day below. The roster supervisor will review and update it.
                            </p>
                          </div>
                          ...
                          <form 
                            onSubmit={(e) => {
                              ...
                              const proposedDay = (form.elements.namedItem('proposed_day') as HTMLSelectElement).value;
                              handleRequestWeekOffChange(currentEmployee.id, proposedDay);
                            }}
                            className="space-y-4"
                          >
                            <div className="space-y-1">
                              <label className="block text-[10px] font-bold text-slate-500 uppercase">Proposed Day Off</label>
                              <select
                                name="proposed_day"
                                defaultValue={DAYS_OF_WEEK.find(d => d !== currentEmployee.weekOff) || 'Sunday'}
                                className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold focus:bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500/10 cursor-pointer"
                              >
                                {DAYS_OF_WEEK.map(day => (
                                  <option key={day} value={day}>
                                    {day} {day === currentEmployee.weekOff ? '(Current)' : ''}
                                  </option>
                                ))}
                              </select>
                            </div>
                            ...
                            <button
                              type="submit"
                              className="w-full py-2.5 px-4 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl text-xs shadow-xs transition-all cursor-pointer flex items-center justify-center gap-1.5"
                            >
                              <Check size={14} />
                        ...
                        {/* C. Recent Submissions Status Tracking */}
                        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs flex flex-col justify-between">
                          <div>
                            <h3 className="text-sm font-bold text-slate-800 flex items-center gap-2">
                              <History size={15} className="text-emerald-700" />
                              <span>Your Requests Status</span>
                            </h3>
                            <p className="text-xs text-slate-500 mt-1">
                              Live tracking of weekly off days filled for approval.
                            </p>
                          </div>

                          <div className="mt-3 flex-1 overflow-y-auto max-h-40 divide-y divide-slate-100 pr-1 space-y-2.5">
                            {requests.filter(r => r.empId === currentEmployee.id).length === 0 ? (
                              <div className="py-6 text-center text-slate-400 text-xs">
                                No previous requests found.
                              </div>
                            ) : (
                              requests.filter(r => r.empId === currentEmployee.id).map(req => (
                                <div key={req.id} className="pt-2 text-xs flex items-center justify-between gap-2">
                                  <div>
                                    <div className="font-bold text-slate-800 font-sans">Change to {req.proposedWeekOff}</div>
                                  ...
                                  <div>
                                    {req.status === 'pending' && (
                                      <span className="text-[10px] bg-amber-50 text-amber-700 border border-amber-200 font-bold px-2.5 py-0.5 rounded-full uppercase tracking-wider">
                                        Pending
                                      </span>
                                    )}
                                    {req.status === 'approved' && (
                                      <span className="text-[10px] bg-emerald-50 text-emerald-800 border border-emerald-200 font-bold px-2.5 py-0.5 rounded-full uppercase tracking-wider">
                                        Approved
                                      </span>
                                    )}
                                    {req.status === 'rejected' && (
                                      <span className="text-[10px] bg-rose-50 text-rose-800 border border-rose-200 font-bold px-2.5 py-0.5 rounded-full uppercase tracking-wider">
                                        Declined
                                      </span>
                                    )}
                                  </div>
